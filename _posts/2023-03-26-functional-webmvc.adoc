...
I don't understand why spring developers king using RestControllers when a nicer way exists.
...
It's a much nicer way to the all the routes especially when there are many controller.
...
It's pretty straight forward and similar to the code from the RestController we started with. The thing missing is the api routes.
